...
interface CreateUser extends Omit<Register, 'password'> {
    password?: string
    title?: string,
    phone?: string,
    picture?: string,
...
export class AccountStore implements Store {
    ...
    async updateProfile(id: string, payload: UpdateProfile): Promise<User> {
        ...
        if (payload['first_name']) query.set('first_name', payload['first_name'])
        if (payload['last_name']) query.set('last_name', payload['last_name'])
        ...
        if (payload['picture']) query.set('picture', payload['picture'])
