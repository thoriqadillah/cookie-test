...
export class EmailNotifier implements Notifier {
    ...
    async send(recepients: string[], option?: SendOption): Promise<void> {
        ...
        if (option?.templatePath) {
            template = fs.readFileSync(path.join(__dirname, `templates/${option.templatePath}`), 'utf8');
        }
        ...
        await this.transport.sendMail({
            ...
            text: option?.message,
            html: option?.templatePath ? mustache.render(template, option.payload) : undefined
        })
    }
}